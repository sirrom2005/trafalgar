...
class AddDestination extends common {
    ...
    protected function _config() {
        parent::_config();
        $this->autoAppendView = 'add.specials.view.php';  // set page view file name
    }
    ...
    protected function _load() {
        if($this->id){
            ...
            $rs = $this->db->table('specials', 'id=?', $this->id);
    ...
    public function sumbit_data() {   
        ...
        $img                = $this->uploadImage('specials');
        if($img){$data['image'] = $img;}
                     
        if($this->id)
        {
            if($img){
                $rs = $this->db->table('view_get_special', 'id=?', $this->id);           
                if($rs[0]['image']){
                    $this->delImage($rs[0]['image']);
                }
            }
            $this->db->tableUpdate('specials', $data, 'id=?', $this->id);
            $this->writeApi(API_SPECIALS);
            $this->callScriptFn('callModal');
        }else{
            $data['date_added'] = $this->dateNow;
            $this->db->tableInsert('specials', $data);
            $this->writeApi(API_SPECIALS);
